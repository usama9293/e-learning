...
const MaterialsTabContent = ({ selectedCourse }) => {
...
const handleFileChange = (event) => {
  setMaterialFile(Array.from(event.target.files));
};
...
const handleSubmit = async () => {
  ...
  materialFile.forEach((file) => {
    formData.append("files", file);  // key name must match FastAPI param
  });
  try {
  ...
  const handleDelete = async (id) => {
    if (confirm("Are you sure you want to delete this material?")) {
      try {
        ...
        // refresh list
        fetchMaterials(); // or filter it out from state
      } catch (err) {
        console.error(err);
  ...
  <DialogTitle>
    Add Material for {selectedCourse.course.name }
    <IconButton
      aria-label="close"
